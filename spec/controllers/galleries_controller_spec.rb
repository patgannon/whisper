require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by the Rails when you ran the scaffold generator.

describe GalleriesController do
  before :each do
    create_default_project
  end

  def mock_gallery(stubs={})
    @mock_gallery ||= mock_model(Gallery, stubs).as_null_object
    if stubs[:save] == false
      @mock_gallery.stub_chain(:errors, :empty?) { false }
    end
    @mock_gallery
  end

  describe "GET index" do
    it "assigns all galleries as @galleries" do
      @project.stub(:galleries) { [mock_gallery] }
      get :index
    end
  end

  describe "GET show" do
    it "assigns the requested gallery as @gallery" do
      Gallery.stub(:find).with("37") { mock_gallery }
      get :show, :id => "37"
      assigns(:gallery).should be(mock_gallery)
    end
  end

  describe "GET new" do
    it "assigns a new gallery as @gallery" do
      Gallery.stub(:new) { mock_gallery }
      get :new
      assigns(:gallery).should be(mock_gallery)
    end
  end

  describe "GET edit" do
    it "assigns the requested gallery as @gallery" do
      Gallery.stub(:find).with("37") { mock_gallery }
      get :edit, :id => "37"
      assigns(:gallery).should be(mock_gallery)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "assigns a newly created gallery as @gallery" do
        @project.galleries.stub(:build).with({'these' => 'params'}) { mock_gallery(:save => true) }
        post :create, :gallery => {'these' => 'params'}
        assigns(:gallery).should be(mock_gallery)
      end

      it "redirects to the created gallery" do
        @project.galleries.stub(:build) { mock_gallery(:save => true) }
        post :create, :gallery => {}
        response.should redirect_to(gallery_url(mock_gallery))
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved gallery as @gallery" do
        @project.galleries.stub(:build).with({'these' => 'params'}) { mock_gallery(:save => false) }
        post :create, :gallery => {'these' => 'params'}
        assigns(:gallery).should be(mock_gallery)
      end

      it "re-renders the 'new' template" do
        @project.galleries.stub(:build) { mock_gallery(:save => false) }
        post :create, :gallery => {}
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested gallery" do
        Gallery.stub(:find).with("37") { mock_gallery }
        mock_gallery.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :gallery => {'these' => 'params'}
      end

      it "assigns the requested gallery as @gallery" do
        Gallery.stub(:find) { mock_gallery(:update_attributes => true) }
        put :update, :id => "1"
        assigns(:gallery).should be(mock_gallery)
      end

      it "redirects to the gallery" do
        Gallery.stub(:find) { mock_gallery(:update_attributes => true) }
        put :update, :id => "1"
        response.should redirect_to(gallery_url(mock_gallery))
      end
    end

    describe "with invalid params" do
      it "assigns the gallery as @gallery" do
        Gallery.stub(:find) { mock_gallery(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:gallery).should be(mock_gallery)
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested gallery" do
      Gallery.stub(:find).with("37") { mock_gallery }
      mock_gallery.should_receive(:destroy)
      delete :destroy, :id => "37"
    end

    it "redirects to the galleries list" do
      Gallery.stub(:find) { mock_gallery }
      delete :destroy, :id => "1"
      response.should redirect_to(galleries_url)
    end
  end

end
